	...
	try:
		...
		if seller is not None:
			if id == 0:
				...
				pageitems = seller.beordered.filter(or_("paystate = 1" , "paystate = 7")).paginate(page, per_page = 3, error_out = False)
			else:
				pageitems = seller.beordered.filter(or_("paystate = 2", "paystate = 6")).paginate(page, per_page = 3, error_out = False)
			headImg = ''
...
def customerOrder(id):
	#try:
		...
		if customer is not None:
			if id == 0:
				pageitems = customer.order.filter("paystate = 0").paginate(page, per_page = 3, error_out = False)
			elif id == 1:
				pageitems = customer.order.filter(or_("paystate = 1" , "paystate = 7")).paginate(page, per_page = 3, error_out = False)
			else:
				pageitems = customer.order.filter(or_("paystate = 2", "paystate = 6")).paginate(page, per_page = 3, error_out = False)
